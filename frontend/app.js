...
function formatSourceProduct(product, error, excludedOfficialStoreCount = 0) {
  if (error) {
  ...
  const lines = [
    ...
    `Price: ${product.currency || ""} ${product.price || "n/a"}`.trim(),
    `Material: ${product.material || "n/a"}`,
    `Features: ${(product.features || []).join(", ") || "n/a"}`,
  ];

  if (excludedOfficialStoreCount > 0) {
    lines.push(`Excluded official stores: ${excludedOfficialStoreCount}`);
  }
...
function renderMatches(matchesNode, topMatches) {
  ...
  topMatches.forEach((match, index) => {
    ...
    matchFragment.querySelector(".match-header").innerHTML = `
      <strong>Rank ${index + 1} - ${match.marketplace}</strong><br />
...
function renderActivityLog(entries) {
  ...
  if (activityLogNode.dataset.renderedEntries === fingerprint) {
    return;
  }

  activityLogNode.dataset.renderedEntries = fingerprint;
  activityLogNode.innerHTML = "";
  ...
  entries
    ...
    .forEach((entry) => {
      ...
      name.textContent = entry.agent_name;
      ...
      if (entry.metadata && Object.keys(entry.metadata).length > 0) {
        ...
        metaCode.textContent = JSON.stringify(entry.metadata, null, 2);
        meta.append(metaCode);
        item.append(meta);
      }

      activityLogNode.appendChild(item);
    });
}
  ...
  setInnerHtml(
    ...
    `
      ...
      ${formatSourceProduct(
        ...
        report.error,
        report.excluded_official_store_count || 0
      )}
    `
  );
